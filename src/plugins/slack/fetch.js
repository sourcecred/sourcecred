...
export class Fetcher {
  ...
  async channels(): Promise<Array<Conversation>> {
    ...
    do {
      try {
        response = await this.web.conversations.list({ limit: 100, types, cursor });
      } catch (error) {
  ...
  async getRequiredDetails(message: any, channel: Conversation): Promise<Message> {
    let block: any;
    const mentioned_users: any = [];
    ...
    if (message.blocks) {
      ...
      if (innerElements) {
        const mentioned_users_filter = innerElements.filter((el) => el.type == 'user');
        for (let user of mentioned_users_filter) {
          mentioned_users.push(user.user_id);
        }
      }
    }
  ...
  async getAllReplies(timestamp: any, channel: Conversation): Promise<Array<Message>> {
    let intermediateResult: any, cursor: any = '';
    let allReplies: any = [];
    do {
      try {
  ...
  async messages(channel: Conversation): Promise<Array<Message>> {
    ...
    do {
      try {
        response = await this.web.conversations.history({ channel: channel.id, cursor });
      } catch (error) {
        throw new Error ("Error in fetching messages");
      }
      const userMessages = response.messages.filter((message) => !(message.bot_id));

      // clean the message obj and fetch replies if any
      for (const msg of userMessages) {
        let requiredDetails = await this.getRequiredDetails(msg, channel);
