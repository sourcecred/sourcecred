 ...
 *
 * We wrap so that we can dependency inject saveMetadata for testability.
 * (saveMetadata depends on the directory state that's generated by running
 * load.)
 */
export const makeLoadDefaultPlugins = (saveMetadata: SaveMetadata) => {
  return async (options: LoadOptions) => {
    ...
    await saveMetadata(defaultAdapterLoaders(), options.output);
...
/**
 * Save AnalysisAdapter metadata after the repos are loaded.
 *
 * Takes the AnalysisAdapterLoaders and RepoId, and saves createdAt and
 * description information for given repo.
 *
...
export async function saveMetadata(
  ...
  const descriptionMap = createDescriptionMap(graph.nodes(), adapters);
  writeDescriptionMap(descriptionMap, Common.sourcecredDirectory(), repoId);
}
