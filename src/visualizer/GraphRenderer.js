...
/**
 * A Graph Renderer which straddles the boundary between React and D3.
 *
 ...
 */
export class GraphRenderer extends React.Component<Props> {
  render() {
    const NodeRenderers = this.props.nodes.map((n) => (
      <NodeRenderer
        positionedNode={n}
        key={n.node.address}
    ...
    const EdgeRenderers = this.props.edges.map(({src, dst, address}) => (
      <EdgeRenderer
        key={address}
        ...
        >
          <g
            ...
            transform={`translate(${this.props.size.width / 2}, ${this.props
              .size.height / 2})`}
          >
            <g className="edges">{EdgeRenderers}</g>
            <g className="nodes">{NodeRenderers}</g>
          </g>
        </svg>
        <div
